...
with left_col:
    if st.session_state.pravy_vyber == "kocka1":
        ...
        with tab1:
            ...
            st.info(f"Materiál mustí být odeslán nejpozději dne: **{konecna_lhuta.strftime('%d. %m. %Y')}**")
        ...
        with tab2:
            ...
            preklady = {
                "Je to totální idiot a kompletně si vymýšlí": 
                    "👉 *Tvrzení protistrany vykazují zřejmé známky argumentační nouze, jsou zcela účelová a v příkrém rozporu se skutečným stavem věci...*",
                "Dluží mi prachy, nereaguje and dělá mrtvého brouka": 
                    "👉 *Žalovaný je dlouhodobě v prodlení se splněním svého peněžitého závazku. Na opakované mimosoudní předžalobní výzvy...*",
                "Ta jeho práce stojí za hovno, je to celé nakřivo": 
                    "👉 *Předmět díla vykazuje zjevné statické, technické a estetické vady neodpovídající schválené projektové dokumentaci...*",
                "Už mě neser, nebo tě poženu k soudu a zaplatíš i mýho právníka": 
                    "👉 *V případě, že ze strany žalovaného nedojde k okamžité nápravě tohoto protiprávního stavu, je můj klient nucen hájit svá práva...*"
            }
        ...
        with tab6:
            ...
            if not st.session_state.ukoly[vybrany_makac]:
                ...
                # --- UPRAVENÁ SEKCÍ PRO SMAZÁNÍ S HESLEM ---
                st.markdown("---")
                st.markdown("##### Odstranění dokončených úkolů")
                del_col1, del_col2 = st.columns([2, 1])
                smazat_heslo = del_col2.text_input("Heslo pro potvrzení smazání:", type="password", key=f"del_heslo_{vybrany_makac}")
                ...
                if del_col1.button(f"🗑️ Smazat hotové úkoly ({vybrany_makac})", key=f"btn_smazat_{vybrany_makac}", use_container_width=True):
                    if smazat_heslo == "1234":
                        ...
                        uloz_ukoly()
                        st.success(f"Hotové úkoly pro uživatele {vybrany_makac} byly odstraněny!")
                        st.rerun()
                    else:
                        st.error("❌ Špatné heslo! Úkoly nebyly smazány.")
        ...
        with tab7:
            ...
            if vsechny_ukoly_list:
                ...
                def obarvi_jmena(val):
                    barvy = {
                        "Jaroslav": "color: #d62728; font-weight: bold;",  # Červená
                        "Petr": "color: #1f77b4; font-weight: bold;",      # Modrá
